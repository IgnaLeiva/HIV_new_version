---
title: "Code project HIV - Adherence"
format: html
editor: visual
---
...
dta <- readRDS('data.Rds') |> select(sex, age, VL.dectable, employed = exposure, insurance, comorbidities, migrant, education.level, timeOnART, stgo, housing, self.stigma, addiction, mental.health, adherent)
...
dta |> tbl_summary(by = 'adherent') |> as_gt() |> 
  tab_caption(caption = md("Table 1: Population divided by **adherence status**")) |> 
  tab_footnote(footnote = '0: Non-Adherent, 1: Adherent',placement = "left")
```
...
variables <- names(dta)[!names(dta) %in% c("VL.dectable", 'adherent')]
formula_ps <- as.formula(paste0('adherent ~', paste0(variables, collapse = '+')))
```
...
x <- glm(adherent ~ 1, family = 'binomial', data = dta)
p <- predict(x, type = "response", newdata = dta) %>% unique() 
#z <- prop.table(table(dta$employed))

dta$sw <- ifelse(dta$adherent == 1, p/dta$ps, (1-p)/(1-dta$ps))
```
